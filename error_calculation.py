...
class MeanSquareErrorCalculator:
    ...
    def calculate_mse_normal(self, mean, std_dev):
        p = norm.pdf(list(self.sorted_counter.keys()), mean, std_dev)

        normalized_counter = np.array(list(self.sorted_counter.values())) / self.num_balls
        diff = normalized_counter - p
        squared_diff = diff ** 2
        mse = np.mean(squared_diff)
        return mse

    def calculate_mse_binomial(self):
        binom_pmf = binom.pmf(list(self.sorted_counter.keys()), self.levels, 0.5)

        normalized_counter = np.array(list(self.sorted_counter.values())) / self.num_balls
        diff = normalized_counter - binom_pmf
        squared_diff = diff ** 2
        mse = np.mean(squared_diff)
        return np.sqrt(mse)
